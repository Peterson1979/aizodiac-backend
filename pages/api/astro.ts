...
async function retryWithBackoff<T>(fn: ()=>Promise<T>, retries=MAX_RETRIES): Promise<T> {
  ...
  while(attempt<retries) {
    try { return await fn(); } catch { attempt++; await new Promise(r=>setTimeout(r, Math.pow(2,attempt)*200)); }
  }
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const text = generateResult?.response?.text?.() || generateResult?.text || String(generateResult||"");
