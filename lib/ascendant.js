...
export function calculateAscendant(birthDate, birthTime, latitude, longitude, timezoneOffset = 0) {
  ...
  // ✅ JAVÍTÁS: NE használjunk `new Date()`-ot! Manuális UTC konverzió.
  ...
  // Nap normalizálása (naiv, de elegendő)
  ...
  while (utcDay < 1) {
    ...
    const prevMonthDays = daysInMonth[(utcMonth - 1 + 12) % 12];
    if (utcMonth === 2 && isLeapYear(utcYear)) {
      utcDay += 29;
    } else {
  ...
  let currentMonthDays = daysInMonth[(utcMonth - 1 + 12) % 12];
  if (utcMonth === 2 && isLeapYear(utcYear)) {
    currentMonthDays = 29;
  }

  while (utcDay > currentMonthDays) {
    utcDay -= currentMonthDays;
    utcMonth += 1;
    if (utcMonth > 12) {
      utcMonth = 1;
      utcYear += 1;
    }
    currentMonthDays = daysInMonth[(utcMonth - 1 + 12) % 12];
    if (utcMonth === 2 && isLeapYear(utcYear)) {
      currentMonthDays = 29;
    }
  }
