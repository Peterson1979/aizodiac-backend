...
/**
 * GMST (Greenwich Mean Sidereal Time) - IAU 2006 pontos képlet
 */
function calculateGMST(jd) {
  ...
  // Órák hozzáadása (JD tört része * 360.98564736629)
  const fracDay = (jd % 1.0);
  const hoursInDegrees = fracDay * 360.98564736629;
  
  let gmst = gmst0 + hoursInDegrees;
...
/**
 * ASZCENDENS SZÁMÍTÁS - Jean Meeus "Astronomical Algorithms" alapján
 * Ez a standard, mindenki által használt képlet
 */
function calculateAscendant_FINAL(lst, latitude, jd) {
  ...
  const ramc = lst * Math.PI / 180; // RAMC = Right Ascension of Medium Coeli
  
  // Meeus képlet az aszcendensre
...
export function calculateAscendant(birthDate, birthTime, latitude, longitude) {
  ...
  console.log(`🔍 FULL DEBUG: UTC=${utcYear}-${utcMonth}-${utcDay} ${utcHour}:${minute}, JD=${jd.toFixed(5)}, GMST=${gmst.toFixed(2)}°, LST=${lst.toFixed(2)}°, ASC=${ascDeg.toFixed(2)}° → ${SIGNS[signIndex]}`);
