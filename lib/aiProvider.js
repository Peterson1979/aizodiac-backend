...
export async function executeProviderRouting({
  ...
  geminiModel = process.env.GENERATIVE_MODEL || DEFAULT_GEMINI_MODEL,
}) {
  ...
  const primaryModel = normPrimary === AI_PROVIDERS.GROQ ? groqModel : geminiModel;
  const fallbackModel = normFallback === AI_PROVIDERS.GEMINI ? geminiModel : groqModel;
