...
export function getSharedCacheKey(type, templateData = {}, model = "default") {
  ...
  switch (type) {
    ...
    case "personal_horoscope": {
      // Build canonical normalized semantic object encompassing all prompt-affecting inputs
      ...
      const semanticHash = createHash("sha256")
        ...
        .digest("hex")
        .slice(0, 32);
      return `aiz:cache:${CACHE_VERSION}:${normModel}:personal_horoscope:${semanticHash}:${normLang}`;
    }
    ...
    case "numerology": {
      ...
      return `aiz:cache:${CACHE_VERSION}:${normModel}:numerology:${lp}_${exp}_${su}_${pers}_${bday}:${normLang}`;
    }
    ...
    case "personal_astro_calendar": {
      ...
      return `aiz:cache:${CACHE_VERSION}:${normModel}:personal_astro_calendar:${range}:${d1}_${d2}_${d3}:${normLang}`;
    }
