...
export const CACHE_TYPE_REVISION = {
  home_daily_horoscope: "b7",
  ai_horoscope_daily: "b7",
  ai_horoscope_weekly: "b7",
  ai_horoscope_monthly: "b7",
  ai_horoscope_yearly: "b7",
...
export function getSharedCacheKey(type, templateData = {}, model = "default", provider = "gemini") {
  ...
  switch (type) {
    case "home_daily_horoscope": {
      const rev = CACHE_TYPE_REVISION.home_daily_horoscope;
      const sign = String(templateData.zodiacSign || "unknown").trim().toLowerCase();
      const date = String(templateData.currentDate || "").trim();
      return `aiz:cache:${CACHE_VERSION}:${normModel}:home_daily_horoscope:${rev}:${date}:${sign}:${normLang}`;
    }
    ...
    case "ai_horoscope_general": {
      const rev = CACHE_TYPE_REVISION.ai_horoscope_daily;
      const sign = String(templateData.zodiacSign || "unknown").trim().toLowerCase();
      const date = String(templateData.currentDate || "").trim();
      return `aiz:cache:${CACHE_VERSION}:${normModel}:ai_horoscope_daily:${rev}:${date}:${sign}:${normLang}`;
    }
