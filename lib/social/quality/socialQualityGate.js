...
import { validateWebsiteDestinationUrl } from "../content/destinations.js";
import {
  CANVAS_WIDTH,
  CANVAS_HEIGHT,
 ...
 */
export function validateAiCreative(creative, options = {}) {
  const errors = [];
  const warnings = [];
  ...
  const format = options.format || creative.format || null;
  const isThreeSignRequired = format === "three_zodiac_signs" || (!format && !options.expectedCategory);
  ...
  if (!creative.topic || typeof creative.topic !== "string" || creative.topic.trim().length < 5) {
    ...
    if (isThreeSignRequired && !/\b(?:3|three)\b/i.test(topic)) {
      ...
      if (!item.sign || typeof item.sign !== "string" || item.sign.trim().length === 0) {
        ...
        if (isThreeSignRequired) {
          ...
          if (normSign.length > 40) {
            errors.push(`Item at index ${idx} title exceeds 40 characters (length: ${normSign.length})`);
          }
...
export function validateCanonicalAssembly(canonical, { expectedDate, expectedCategory } = {}) {
  ...
  if (!canonical.pinterestLink || typeof canonical.pinterestLink !== "string") {
    errors.push("Missing or invalid canonical 'pinterestLink'");
  } else {
    const pinVal = validateWebsiteDestinationUrl(canonical.pinterestLink, { expectedPlatform: "pinterest" });
    if (!pinVal.valid && canonical.pinterestLink !== DEFAULT_APP_PLAY_STORE_URL) {
      errors.push(...pinVal.errors.map(e => `[Pinterest Link] ${e}`));
    }
  }
  ...
  if (!Array.isArray(canonical.slides) || canonical.slides.length !== 5) {
    ...
    const isThreeSignRequired = canonical.format === "three_zodiac_signs" || (!canonical.format && canonical.category === "three_zodiac_signs");
    const seenSigns = new Set();
    for (let i = 1; i <= 3; i++) {
      ...
      if (!s || s.type !== "sign") {
        ...
        if (!s.sign || typeof s.sign !== "string" || s.sign.trim().length === 0) {
          errors.push(`Slide ${i + 1} has type 'sign' but missing 'sign' field`);
        } else {
          const normSign = s.sign.trim();
          ...
          if (isThreeSignRequired) {
            if (!VALID_ZODIAC_SIGNS.has(capitalizedSign)) {
              ...
              errors.push(`Slide ${i + 1} repeats sign: '${capitalizedSign}'`);
            } else {
              seenSigns.add(capitalizedSign);
            }
          } else {
            if (normSign.length > 40) {
              errors.push(`Slide ${i + 1} title exceeds 40 characters (length: ${normSign.length})`);
            }
            if (seenSigns.has(normSign.toLowerCase())) {
              errors.push(`Duplicate slide title '${normSign}' found at slide index ${i}`);
            } else {
              seenSigns.add(normSign.toLowerCase());
            }
          }
  ...
  // Mandatory Facebook Google Play Link check
  if (!canonical.facebookCaption || typeof canonical.facebookCaption !== "string") {
    errors.push("Missing 'facebookCaption'");
  } else if (!canonical.facebookCaption.includes(DEFAULT_APP_PLAY_STORE_URL)) {
    errors.push(`Canonical facebookCaption must contain the mandatory Google Play link: '${DEFAULT_APP_PLAY_STORE_URL}'`);
  }
...
export function validatePublishManifest(manifest, { mediaBaseUrl } = {}) {
  ...
  if (!manifest.captions || typeof manifest.captions !== "object") {
    ...
    if (!manifest.captions.facebook || typeof manifest.captions.facebook !== "string" || manifest.captions.facebook.trim().length === 0) {
      errors.push("Missing 'captions.facebook'");
    } else if (!manifest.captions.facebook.includes(DEFAULT_APP_PLAY_STORE_URL)) {
      errors.push(`'captions.facebook' must contain the mandatory Google Play link: '${DEFAULT_APP_PLAY_STORE_URL}'`);
    }
    if (!manifest.captions.pinterest || typeof manifest.captions.pinterest !== "object") {
      ...
      if (!manifest.captions.pinterest.link || typeof manifest.captions.pinterest.link !== "string") {
        errors.push("Missing or invalid 'captions.pinterest.link'");
      } else {
        const pinVal = validateWebsiteDestinationUrl(manifest.captions.pinterest.link, { expectedPlatform: "pinterest" });
        if (!pinVal.valid && manifest.captions.pinterest.link !== DEFAULT_APP_PLAY_STORE_URL) {
          errors.push(...pinVal.errors.map(e => `[Pinterest Link] ${e}`));
        }
      }
    }
