...
export class InstagramAdapter extends BaseSocialAdapter {
  constructor(options = {}) {
    const name = typeof options === "string" ? options : (options.name || options.destinationKey || PLATFORMS.INSTAGRAM);
    super(name);
    this.getAccountId = typeof options.getAccountId === "function"
      ? options.getAccountId
      : (config) => (options.accountId || config?.instagramAccountId);
  ...
  validateConfig(config) {
    ...
    if (!token) errors.push(`Missing Access Token for Instagram (${this.name})`);
    if (!accountId) errors.push(`Missing Instagram Business Account ID for Instagram (${this.name})`);
