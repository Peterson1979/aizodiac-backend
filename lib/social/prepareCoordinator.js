...
export async function executeDailyPreparation({
  ...
  const prepState = await getPrepareState(redis, publishDate);
  ...
  const hasValidPreparedPackage = prepState?.stage === PREPARE_STAGES.QUALITY_GATE_PASS
    && existingManifest?.metadata?.qualityGate === QUALITY_GATE_STATUS.PASS;
  const isExplicitManualOverride = existingManifest?.metadata?.source === "manual_override"
    || existingManifest?.metadata?.manualOverride === true
    || prepState?.manualOverride === true;

  if (existingManifest && !forceRegenerate && (hasValidPreparedPackage || isExplicitManualOverride)) {
    return {
      success: true,
      status: "EXISTING_MANIFEST_FOUND",
      message: hasValidPreparedPackage
        ? `Valid prepared manifest found for date ${publishDate}. Preparation reused idempotently.`
        : `Explicitly approved manual manifest found for date ${publishDate}. AI generation skipped.`,
      publishDate,
      manifestId: existingManifest.id,
      manifest: existingManifest,
      source: hasValidPreparedPackage ? "generated_existing" : "manual_override",
      dryRun,
    };
  }

  const legacyManifestRecovery = Boolean(existingManifest && !forceRegenerate);
  ...
  try {
    ...
    // Reuse the state read before manifest classification.
    ...
    try {
      uploadedMedia = await uploadCarouselSlides({
        ...
        s3Client,
        dryRun,
        overwriteExisting: legacyManifestRecovery,
      });
    ...
    const manifest = {
      ...
      media: manifestMedia,
      destinationUrl: generatedContent.destinationUrl || generatedContent.pinterestLink || DEFAULT_APP_PLAY_STORE_URL,
      destinations: generatedContent.destinations || null,
      captions: {
      ...
      metadata: {
        category: generatedContent.category,
        format: generatedContent.format || generatedContent.category,
        topic: generatedContent.topic,
        contentId: generatedContent.contentId,
        destinationPath: generatedContent.destinationPath || null,
