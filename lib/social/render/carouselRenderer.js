...
import { getTopicStrategyForDate, stripFormatHeadlinePrefix } from "../content/topicRotation.js";
 ...
 * @returns {string}
 */
export function generateSlideAltText(slide, format) {
  if (slide.type === "cover" || slide.type === "title") {
    const rawHeadline = slide.headline || slide.title || "Daily Zodiac Insights";
    const cleanHeadline = stripFormatHeadlinePrefix(rawHeadline, format);
    return `${cleanHeadline} | AI Zodiac`;
  }
  if (slide.type === "cta") {
...
export async function renderCarouselSlides(socialContent, options = {}) {
  ...
  const format = socialContent.format || strategy.format;
  ...
  const renderPromises = socialContent.slides.map(async (slide, idx) => {
    ...
    const buffer = await renderSlidePng({
      ...
      slideNumber,
      totalSlides,
      category: socialContent.category || format,
      categoryTitle,
      format,
    });
    ...
    const altText = generateSlideAltText(slide, format);
