...
export const LAYOUT_ZONES = Object.freeze({
  ...
  COVER: {
    CATEGORY: { top: 220, bottom: 330, height: 110 },
...
export async function renderTextToFit({
  ...
  while (size >= minSize) {
    const pango = `<span foreground="${color}" font_family="Noto Sans" ${isBold ? 'font_weight="bold"' : 'font_weight="normal"'} font_size="${Math.round(size * 1024)}">${escapeXml(formatted || "")}</span>`;
    const textConfig = {
      text: pango,
      font: "Noto Sans",
      fontfile,
