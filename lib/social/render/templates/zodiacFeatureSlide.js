...
export function renderZodiacFeatureSlideOverlaySvg({ sign } = {}) {
  ...
  const glyphSvg = getZodiacSvgGlyph(capitalized, accent.color, 3.5);
...
export async function getZodiacFeatureSlideTextLayers({
  ...
  const arrowSvg = renderVectorArrowSvg({
    color: COLORS.accentLavender,
    width: 28,
    height: 28,
    strokeWidth: 3,
  ...
  composites.push({
    input: arrowBuffer,
    top: Math.round(1185 - 14),
    left: swipeStartLeft + swipeTextRes.width + 12,
  });
