...
export function renderZodiacFeatureSlideOverlaySvg({ sign } = {}) {
  ...
  const accent = data ? data.accent : { color: COLORS.accentLavender, badgeBg: "rgba(196, 181, 253, 0.18)", label: "AI ZODIAC" };
  return `
...
export async function getZodiacFeatureSlideVectorLayers({ sign } = {}) {
  const normSign = (sign || "").trim();
  const capitalized = normSign ? (normSign.charAt(0).toUpperCase() + normSign.slice(1).toLowerCase()) : "Celestial";
  const data = ZODIAC_DATA[capitalized];
  const accentColor = data ? data.accent.color : COLORS.accentLavender;
  const glyphSvg = getZodiacSvgGlyph(capitalized, accentColor, 3.5);
  ...
  return {
    emblem: { input: await sharp(Buffer.from(emblemSvg)).png().toBuffer(), top: 165, left: 450, vectorType: data ? "zodiac-symbol" : "celestial-symbol", sign: capitalized },
 ...
 * @param {object} params
 * @param {string} [params.sign] - Zodiac sign name or slide item title
...
export async function getZodiacFeatureSlideTextLayers({
  ...
  const normSign = (sign || "").trim();
  const capitalized = normSign ? (normSign.charAt(0).toUpperCase() + normSign.slice(1).toLowerCase()) : "Insight";
  const data = ZODIAC_DATA[capitalized];
  const accent = data ? data.accent : {
    color: COLORS.accentLavender,
    badgeBg: "rgba(196, 181, 253, 0.18)",
    label: (categoryTitle || category || "AI ZODIAC").toUpperCase().slice(0, 20),
  };
  ...
  // 3. Sign Name / Item Title (Zone: y 365-440, e.g. TAURUS or THE ASCENDANT)
  const signRes = await renderTextToFit({
    text: (normSign || "INSIGHT").toUpperCase(),
    fontfile: fontBoldPath,
    preferredSize: 56,
    minSize: 36,
  ...
  if (typeof headline !== "string" || headline.trim().length === 0) {
    throw new Error(`Feature slide requires a non-empty headline`);
  }
  if (normSign && headline.trim().toLowerCase() === normSign.toLowerCase()) {
    throw new Error(`Feature slide headline must not equal sign/item title: '${headline}'`);
  }
