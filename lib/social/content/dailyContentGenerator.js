...
export function assembleCanonicalSocialContent({ creative, publishDate, category }) {
  ...
  const destinationPath = resolveWebsitePath({ category: normCategory, publishDate });
  ...
  const formattedInstagramCaption = formatInstagramCaption({
    ...
    websiteUrl: instagramDestinationUrl,
  });
    ...
    try {
      ...
      if (parsed.items && Array.isArray(parsed.items)) {
        ...
        if (!parsed.destinationPath) {
          parsed.destinationPath = resolveWebsitePath({ category: parsed.category, publishDate });
        }
