...
export async function executeSocialPublishing({
  ...
  // NO provider write may occur unless the package has passed the Quality Gate
  const prepState = redis ? await getPrepareState(redis, publishDate) : null;
  const isQualityGatePassed = prepState?.stage === PREPARE_STAGES.QUALITY_GATE_PASS ||
    (manifest?.metadata?.qualityGate === QUALITY_GATE_STATUS.PASS);

  if (!isQualityGatePassed) {
    return {
      success: false,
      status: "QUALITY_GATE_BLOCKED",
      message: `Publication blocked: Quality gate state is '${prepState?.stage || "ABSENT"}', expected '${PREPARE_STAGES.QUALITY_GATE_PASS}'. Zero provider writes committed.`,
      date: publishDate,
